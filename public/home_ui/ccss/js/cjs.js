  ...
  addtocartbtnDom.addEventListener("click", () => {
    ...
    if (IsinCart === false) {
      cartDom.insertAdjacentHTML("beforeend", `
        <div class="row shadow-sm card-deck cart-items mt-2 mb-3">
          ...
          <div class="col-md-1 p-2 mt-3">
            <button class="btn btn-info" type="button" data-action="increase-item">&plus;</button>
          </div>
          <div class="col-md-1 p-2 mt-3">
            <p class="text-success cart_item_quantity">${product.quantity}</p>
          </div>
          <div class="col-md-1 p-2 mt-3">
            <button class="btn btn-primary" type="button" data-action="decrease-item">&minus;</button>
          </div>
          <div class="col-md-1 p-2 mt-3">
            <button class="btn btn-danger" type="button" data-action="remove-item">&times;</button>
          </div>
        </div>
      `);
      ...
      if (document.querySelector('.cart-footer') === null) {
        cartDom.insertAdjacentHTML("afterend", `
          <div class="row shadow-sm card-deck cart-footer mt-2 mb-3">
            <div class="col-md-6 p-2">
              <button class="btn btn-danger" type="button" data-action="clear-cart">Clear Cart</button>
            </div>
            <div class="col-md-6 p-2" >
              <button class="btn btn-success"  type="button" data-action="check-out">Pay <span class="pay"></span> &#10137;</button>
            </div>
          </div>
        `);
      }
      ...
      cartItemsDom.forEach(cartItemDom => {
        if (cartItemDom.querySelector(".cart_item_name").innerText === product.name) {
          ...
          document.querySelector('.pay').innerText = cartTotal + " KSH.";
          ...
          cartItemDom.querySelector('[data-action="increase-item"]').addEventListener("click", () => {
            cart.forEach(cartItem => {
              if (cartItem.name === product.name) {
                ...
                cartItemDom.querySelector(".cart_item_price").innerText = parseInt(cartItem.quantity) * parseInt(cartItem.price) + " KSH.";
...
function checkOut() {
  ...
  cart.forEach((cartItem, index) => {
    ...
    <input type="hidden" name="amount_${index}" value="${cartItem.price.replace("KSH.","")}">
